...
- The user selects an AI provider from the fixed list using arrow key navigation: **GitHub Copilot**, **OpenAI**, **Anthropic**, **Google Gemini**, **Ollama** — followed by a line separator and a **⏭️ Skip — set up later in ⚙️ Settings** option
...
- For OpenAI, Anthropic, and Google Gemini, the user is prompted to enter a preferred model name (pre-filled with the current or default value). Entering an empty string resets the model to the provider's default (`gpt-5.4-mini`, `claude-haiku-4-latest`, or `gemini-2.5-flash` respectively)
